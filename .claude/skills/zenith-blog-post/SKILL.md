...
- `src/components/blog/BlogRelated.astro` — sección "Sigue leyendo" al final de cada post: recomienda otros artículos del sitio, barajados en cliente (aleatorio por visita). Todo post nuevo la incluye: import `BlogRelated` y ponla entre `</main>` y `<BlogSearch>` con `posts={posts} currentSlug="<slug>"`.
