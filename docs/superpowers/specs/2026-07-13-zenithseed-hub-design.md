...
## Contenido del hub (texto ya aportado por el usuario, no se reescribe el fondo — solo se ajusta tono/formato al maquetar)
...
**3. Quién soy**
> De pequeño siempre quise programar videojuegos, y ya desde bien pequeño tuve ese interés en la informática — con no más de 8 años ya desmonté y monté mi propio ordenador. Siempre tuve mucha facilidad gracias al alcance que tuve a equipos informáticos. Entre idas y venidas terminé estudiando el grado superior DAM (Desarrollo de Aplicaciones Multiplataforma). Al principio programar me asustó, y es cierto que no sentí que fuera lo mío al inicio, pero poco a poco fui viendo cómo aplicarlo a las cosas que me gustan, y encontré que me resulta divertido programar. En el ámbito laboral programar a veces me resulta terrible, entre toda la burocracia que rodea cosas que se me escapan por el momento — pero programar videojuegos, mis propias páginas web, encontrar recursos hechos por otros y usar las soluciones que el resto del mundo ya ha planteado mientras lo mezclo con mi propio criterio me resulta divertido, fascinante, emocionante. Me encanta la programación, me encanta cacharrear con equipos, redes, desplegar algún servicio y automatizar con IA. A día de hoy estoy tratando de hacer mis propios videojuegos y apps, porque creo que el arte y las soluciones en el mundo de la informática son uno de los medios más llamativos para avanzar.

Tratamiento: párrafo largo, se puede partir en 2-3 bloques visuales para que respire (no un muro de texto), pero el contenido no se resume ni se recorta.
...
**5. Visión de futuro**
> Este sitio será un mix de todas mis próximas idas y venidas: si el día de mañana me pongo a montar mi propio modelo de IA, aquí se verá reflejado; si creo un modelo 3D, si encuentro algo fascinante que comentar — todo eso a lo que me lance a descubrir lo iré recopilando aquí. Esto será un diario personal técnico para bajar a tierra conceptos que desde fuera parecen complejos. Es mi rincón de estudiante/trabajador.
...
- Espaciado de sección generoso: 120–180px verticales (vs. 60–80px del porfolio) — ritmo editorial, no dashboard denso.
...
**No se adopta de Hyperstudio:** su paleta fija, ni retirar el accent color (se mantiene, pero disciplinado — reservado a estados activos/hover/links, no decorativo).
...
Un solo motor WebGL (no se combina con `Dither`/three.js — evita correr dos contextos WebGL a la vez, ya hubo problemas de rendimiento con Silk+partículas en el porfolio). Se usa `FaultyTerminal` (`ogl`, ligero) con su prop `dither` activado para capturar la estética pixelada que se pedía de `Dither` sin pagar el coste de un segundo renderer.
...
- Capa con `opacity` reducida vía wrapper (similar a como se hizo con `Silk` al 0.12, aquí probablemente algo más visible dado que es el único fondo del hub — se ajusta en implementación mirando contraste real con el texto encima).
